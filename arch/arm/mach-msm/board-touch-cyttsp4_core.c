...
#define TMA400_GPIO_TSP_INT   27	//27
#define CYTTSP4_I2C_IRQ_GPIO 27 	//27 /* J6.9, C19, GPMC_AD14/GPIO_38 */
...
static int cyttsp4_hw_power(int on, int use_irq, int irq_gpio)
{
	...
	if (on) {
		if(verg_en==1){
			printk(KERN_ERR "%s already enable\n", __func__);
			return 0;			
		}
		...
		if(verg_en==0){
			printk(KERN_ERR "%s already disable\n", __func__);
			return 0;			
		}
