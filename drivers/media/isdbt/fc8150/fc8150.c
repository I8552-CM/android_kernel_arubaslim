...
#define GPIO_ISDBT_EN		58 // In Roy, ISDBT_EN pin is changed to GPIO 58
#define GPIO_ISDBT_INT		82
#define GPIO_ISDBT_IRQ		MSM_GPIO_TO_INT(GPIO_ISDBT_INT)

#define SJINU_GPIO_TEMP 1
...
void isdbt_hw_init(void)
{
	...
	/* WHEN using EBI2 CLK should be enabled */
...
void isdbt_hw_deinit(void)
{
	...
	/* WHEN using EBI2 CLK should be disabled */
	clk_disable(isdbt_clk);
...
ssize_t isdbt_read(struct file *filp, char *buf, size_t count, loff_t *f_pos)
{
	...
	//PRINTF(hInit, "avail = %d...len = %d.....count = %d\n", avail, len, count);
#endif
...
static int isdbt_probe(struct platform_device *pdev)
{
	...
	isdbt_clk = clk_get(&pdev->dev, "ebi2_clk");  // SJINU_DEBUG
	if (unlikely(IS_ERR(isdbt_clk)))
	{
		printk("(%s) error of getting EBI2_CLK \n", __func__);
		return PTR_ERR(isdbt_clk);
	}

	clkrate = clk_get_rate(isdbt_clk);
	printk("(%s) clock rate=%ld \n", __func__, clkrate);

	hInit = kmalloc(sizeof(struct ISDBT_INIT_INFO_T), GFP_KERNEL);

	res = BBM_HOSTIF_SELECT(hInit, BBM_PPI); //latin_mm lkj in case of Qualcomm, SPI isn't used BBM_SPI->BBM_PPI
