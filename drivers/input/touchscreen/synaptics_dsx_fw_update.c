...
#define MAIN_FW_IMAGE_NAME "tsp_synaptics/w789/Hennesy_MAIN-E001-PR1296149-DS4.3.5.1.img"
#define SUB_FW_IMAGE_NAME "tsp_synaptics/w789/Hennesy_SUB-E001-PR1296149-DS4.3.5.1.img"
#endif
...
#if DUAL_TSP
u8 FW_KERNEL_MAIN_VERSION[5] = {0, };
u8 FW_KERNEL_SUB_VERSION[5] = {0, };
u8 FW_IC_MAIN_VERSION[5] = {0, };
u8 FW_IC_SUB_VERSION[5] = {0, };
#else
...
bool in_fw_update = false;
EXPORT_SYMBOL(in_fw_update);
...
static int fwu_check_version(void)
{
	...
	printk(KERN_ERR "enter fwu_check_version function..\n");
	/* device firmware id */
	...
	dev_info(&i2c_client->dev,
			...
			fwu->config_data[3]);
#if DUAL_TSP
	if(gpio_get_value(GPIO_TSP_SEL)) {
		FW_KERNEL_SUB_VERSION[0] = fwu->config_data[0];
		FW_KERNEL_SUB_VERSION[1] = fwu->config_data[1];
		FW_KERNEL_SUB_VERSION[2] = fwu->config_data[2];
		FW_KERNEL_SUB_VERSION[3] = fwu->config_data[3];
		strncpy(FW_IC_SUB_VERSION, config_id, 4);
		is_need_update = strcmp(FW_KERNEL_SUB_VERSION,FW_IC_SUB_VERSION);
		printk(KERN_ERR "fwu_check_version    IC ver: %s; SUB ver: %s ,  %d\n",(char *)FW_IC_SUB_VERSION,(char *)FW_KERNEL_SUB_VERSION,is_need_update);
	}
	else {		
		FW_KERNEL_MAIN_VERSION[0] = fwu->config_data[0];
		FW_KERNEL_MAIN_VERSION[1] = fwu->config_data[1];
		FW_KERNEL_MAIN_VERSION[2] = fwu->config_data[2];
		FW_KERNEL_MAIN_VERSION[3] = fwu->config_data[3];
		strncpy(FW_IC_MAIN_VERSION, config_id, 4);
		is_need_update = strcmp(FW_KERNEL_MAIN_VERSION,FW_IC_MAIN_VERSION);
		printk(KERN_ERR "fwu_check_version    IC ver: %s; MAIN ver: %s ,  %d\n",(char *)FW_IC_MAIN_VERSION,(char *)FW_KERNEL_MAIN_VERSION,is_need_update);
	}
#else
	FW_KERNEL_VERSION[0] = fwu->config_data[0];
	FW_KERNEL_VERSION[1] = fwu->config_data[1];
	FW_KERNEL_VERSION[2] = fwu->config_data[2];
	FW_KERNEL_VERSION[3] = fwu->config_data[3];
	strncpy(FW_IC_VERSION, config_id, 4);
	is_need_update = strcmp(FW_KERNEL_VERSION,FW_IC_VERSION);
	printk(KERN_ERR "fwu_check_version    IC ver: %s; kernel ver: %s , %d\n",(char *)FW_IC_VERSION,(char *)FW_KERNEL_VERSION,is_need_update);
#endif
...
static int fwu_start_reflash(void)
{
...
#if DUAL_TSP
		if(gpio_get_value(GPIO_TSP_SEL)) {
			retval = request_firmware(&fw_entry, SUB_FW_IMAGE_NAME,
					&fwu->rmi4_data->i2c_client->dev);
			if (retval != 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Firmware image %s not available\n",
						__func__, MAIN_FW_IMAGE_NAME);
				retval = -EINVAL;
				goto exit;
			}
		} else {
			retval = request_firmware(&fw_entry, MAIN_FW_IMAGE_NAME,
					&fwu->rmi4_data->i2c_client->dev);
			if (retval != 0) {
				dev_err(&fwu->rmi4_data->i2c_client->dev,
						"%s: Firmware image %s not available\n",
						__func__, SUB_FW_IMAGE_NAME);
				retval = -EINVAL;
				goto exit;
			}
		}
	...
	printk(KERN_ERR "fwu_check_version    retavl = %d;\n",retval);
	//if (retval <= 0)
	//	goto exit;
...
{
	printk(KERN_ERR "enter synaptics_fw_updater function \n");
	int retval;
