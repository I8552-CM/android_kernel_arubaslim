/* drivers/input/touchscreen/synaptics_s2200.c
 *
...
#include <linux/platform_data/synaptics_s2200.h>
...
//#include "../../../arch/arm/mach-omap2/sec_common.h"     /// Board file
//#include "../../../arch/arm/mach-msm/board-roy.c"     /// Board file
...
static void set_page (struct ts_data *ts, char page)
{
	u8 command; 

	command = page;	
	ts_write_reg_data(ts->client, 0xFF, &command, 1);
...
#if CONTROL_JITTER
#define F11_2D_CTRL			0x3D				// 2D Command Address
...
static void set_jitter_filter(struct ts_data *ts, bool on)
{
	...
	if (on) {
		command = 0x09;
		prev_jitter = true;
		pr_info("tsp: %s: jitter filter on.\n", __func__);
	} else {
		command = 0x01;
		prev_jitter = false;
		pr_info("tsp: %s: jitter filter off.\n", __func__);
	}
...
static bool fw_updater(struct ts_data *ts, char *mode)
{
	...
	if (!strcmp("force", mode)) {
		...
		if (ts_read_reg_data(ts->client, get_reg_address(ts->client, FW_ADDRESS), buf, 4) > 0) {
...
//		if (strcmp(FW_KERNEL_VERSION, FW_IC_VERSION) > 0) {
		if (strcmp(FW_KERNEL_VERSION, FW_IC_VERSION)) {
...
#define REG_DEVICE_STATUS		0x13							 	// Device Status
#define REG_INT_STATUS			0x14							 	// Interrupt Status
#define REG_FINGER_STATUS		0x15								// Finger State
#define REG_POINT_INFO			0x17								// Postion
#define REG_KEY_INFO			0x00								// Postion
...
static irqreturn_t ts_irq_handler(int irq, void *handle)
{
	...
	if (ts_read_reg_data(ts->client, REG_DEVICE_STATUS, &buf, 1) < 0) {
		printk("tsp: ts_irq_event: i2c failed\n");
		return IRQ_HANDLED;
	}

	if ((buf & 0x0F) == 0x03) {
		printk("tsp: ts_irq_event: esd detect\n");
		reset_tsp(ts);
		return IRQ_HANDLED;
	}

	if (ts_read_reg_data(ts->client, REG_INT_STATUS, &buf, 1) < 0) {
		printk("tsp: ts_irq_event: i2c failed\n");
		return IRQ_HANDLED;
	}

	if ((buf & 0x0F) == 0x04) {
//		printk("tsp: ts_irq_event: 2D event detect\n"); // touchscreen
		int_status = 1;
		
	}
	else if ((buf & 0xF0) == 0x10) {
//		printk("tsp: ts_irq_event: 0D event detect\n"); // button
		int_status = 2;
	}
...
#if TRACKING_COORD
//			printk("tsp: finger state regigster %.2x, %.2x, %.2x\n", state[0], state[1], state[2]);
#endif
			for (i = 0, id = 0; i < 3; i++) {
...
//						pr_err("tsp: read_points: read point failed\n");
						printk("tsp: read_points: read point failed\n");						
						return IRQ_HANDLED;
					}
...
#if TRACKING_COORD
						printk("tsp: finger %d up (%d, %d, %d)\n", id, x, y, point[4]);
#else
//						pr_info("tsp: finger %d up\n", id);
						printk("tsp: finger %d up\n", id);
#endif
...
#if TRACKING_COORD
						printk("tsp: finger %d down (%d, %d, %d)\n", id, x, y, point[4]);
#else
//						pr_info("tsp: finger %d down\n", id);
						printk("tsp: finger %d down\n", id);
#endif
						ts->finger_state[id] = 1;
					}
					input_mt_slot(ts->input_dev, id);
					input_mt_report_slot_state(ts->input_dev, MT_TOOL_FINGER, true);
					input_report_abs(ts->input_dev, ABS_MT_TOUCH_MAJOR, point[3]);
					input_report_abs(ts->input_dev, ABS_MT_PRESSURE, point[4]);
					input_report_abs(ts->input_dev, ABS_MT_POSITION_X, x);
					input_report_abs(ts->input_dev, ABS_MT_POSITION_Y, y);
#if TRACKING_COORD
				//	printk("tsp: finger %d drag (%d, %d, %d)\n", id, x, y, point[4]);
#endif
				}
			}
		break;
		case 2:					// 0d event
			set_page(ts, 0x02); 		// page change
			
			if (ts_read_reg_data(ts->client, REG_KEY_INFO, state, 1) < 0) {
//				pr_err("tsp: ts_irq_event: i2c failed\n");
				printk("tsp: ts_irq_event: i2c failed\n");
				return IRQ_HANDLED;
			}
...
// have to fix
			...
			{		
				...
				/* bit shift to get button's status */		
					...
					button_status = ((state[button_reg] >> button_shift)	& 0x01) != 0;		
//					printk("tsp : Button %d (code %d)\n",button,button_status);		

					if (button == 0x0) {
//						gMenuKey_Intensity = buf[i + 5];
						if(menu_pressed != button_status) {
							input_report_key(ts->input_dev, KEY_MENU, button_status ? PRESS_KEY : RELEASE_KEY);
#if TRACKING_COORD
							printk("tsp : menu key: %s\n", button_status ? "PRESS" : "RELEASE");
#endif					
						}
				...
				const struct i2c_device_id *id)
{
	...
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_X, 0,  ts->platform_data->x_pixel_size, 0, 0);
	input_set_abs_params(ts->input_dev, ABS_MT_POSITION_Y, 0,  ts->platform_data->y_pixel_size, 0, 0);
	...
	printk("succeed to register input device\n");

	gpio_request(GPIO_TOUCH_IRQ, "tsp_irq");
	...
	if(ts_read_reg_data(ts->client, get_reg_address(ts->client, FW_ADDRESS), buf, 4)<0)
	{
		printk("tsp : touch device is NOT connected\n");
		goto err_input_register_device_failed;
	}
	else
		printk("tsp : touch device is connected\n");
...
MODULE_DESCRIPTION("Driver for Synaptics S2200 Touchscreen Controller");
MODULE_AUTHOR(" ");
MODULE_LICENSE("GPL");
