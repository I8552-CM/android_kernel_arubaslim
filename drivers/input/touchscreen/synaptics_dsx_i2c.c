...
#define FLIP_OPEN		1
#define FLIP_CLOSE		0	
#define GPIO_HALL_SW	35
struct synaptics_rmi4_data *copy_data = NULL;

// TSP	initialization fail
static int Tsp_probe_passed = 0;
...
static struct muti_touch_info g_Mtouch_info[MAX_NUMBER_OF_FINGERS];
		...
		struct synaptics_rmi4_fn *fhandler)
{
	...
	for (finger = 0; finger < fingers_supported; finger++) {
		...
		if (finger_status) {
			...
			g_Mtouch_info[finger].posX = x;
			g_Mtouch_info[finger].posY = y;
			g_Mtouch_info[finger].strength = wx;
			g_Mtouch_info[finger].width = wy;
			
			if(g_Mtouch_info[finger].status == 0)
				dev_err(&rmi4_data->i2c_client->dev,
						" Finger %d:"
						"status = 0x%02x"
						"x = %d"
...
#endif
			g_Mtouch_info[finger].status = 1;
			touch_count++;
		}else {
			if (g_Mtouch_info[finger].status == 1) {
				g_Mtouch_info[finger].posX = 0;
				g_Mtouch_info[finger].posY = 0;
				g_Mtouch_info[finger].strength = -1;
				g_Mtouch_info[finger].width = 0;
				dev_err(&rmi4_data->i2c_client->dev," Finger %d:status = 0x%02x", finger, finger_status);
				input_mt_slot(rmi4_data->input_dev, finger);
				input_mt_report_slot_state(rmi4_data->input_dev,
					MT_TOOL_FINGER, false);
				g_Mtouch_info[finger].status = 0;
			}
		}
	}
		...
		struct synaptics_rmi4_fn *fhandler)
{
 ...
 static void release_all_fingers(struct synaptics_rmi4_data *rmi4_data)
 {
	 int i;
	 printk(KERN_ERR "%s start.\n", __func__);
	 for (i = 0; i < MAX_NUMBER_OF_FINGERS; i++) {
		 if (0 == g_Mtouch_info[i].status) {
			 g_Mtouch_info[i].posX = 0;
			 g_Mtouch_info[i].posY = 0;
			 continue;
		 }
		 printk(KERN_ERR "%s %s(%d)\n", __func__,
				 rmi4_data->input_dev->name, i);
		 input_mt_slot(rmi4_data->input_dev, i);
		 input_mt_report_slot_state(rmi4_data->input_dev,
							 MT_TOOL_FINGER, false);
		 g_Mtouch_info[i].posX = 0;
		 g_Mtouch_info[i].posY = 0;
		 if (1 == g_Mtouch_info[i].status)
...
{
	printk("==============samsung_switching_tsp===============");
	extern in_fw_update;
	if (Tsp_probe_passed == 0) {
		printk("[TSP] %s exit: Tsp_probe_passed=%d", __func__ ,Tsp_probe_passed);
		return;
	}
	printk("[TSP]+++++ %s\n", __FUNCTION__);
	printk("[TSP] Flip_status_tsp:%d, flip:%d(hallSW:%d)\n", Flip_status_tsp, flip, gpio_get_value(GPIO_HALL_SW));
	printk( "[TSP] tspsel:%d, current addr:  \n", gpio_get_value(GPIO_TSP_SEL) );

	if (Flip_status_tsp == FLIP_NOTINIT) {
		Flip_status_tsp = flip;
		if(gpio_get_value(GPIO_HALL_SW))
			gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
		else
			gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toSUB);
		printk("[TSP] start %s, Flip_status_tsp:%d, GPIO:%d\n", __FUNCTION__, Flip_status_tsp, gpio_get_value(GPIO_TSP_SEL));
		return;
	}
 
	Flip_status_tsp = flip;
	if(in_fw_update) {
		printk("[TSP] %s, it is in firmware update,miss flip \n",__FUNCTION__);
		return;
	}
	if (Flip_status_tsp == FLIP_OPEN) {
		//suspend the sub screen
		gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toSUB);
		printk("[TSP]suspend the sub screen, gpio_tsp_sel = %d\n",gpio_get_value(GPIO_TSP_SEL));
		synaptics_rmi4_early_suspend(&(copy_data->early_suspend));
		//resume the main screen
		gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
		printk("[TSP]resume the main screen, gpio_tsp_sel = %d\n",gpio_get_value(GPIO_TSP_SEL));
		synaptics_rmi4_late_resume(&(copy_data->early_suspend)); 
	} else {
		//suspend the main screen
		gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
		printk("[TSP]suspend the main screen, gpio_tsp_sel = %d\n",gpio_get_value(GPIO_TSP_SEL));
		synaptics_rmi4_early_suspend(&(copy_data->early_suspend));
		//resume the main screen
		gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toSUB);
		printk("[TSP]resume the sub screen, gpio_tsp_sel = %d\n",gpio_get_value(GPIO_TSP_SEL));
		synaptics_rmi4_late_resume(&(copy_data->early_suspend));  
	}
	return;
}
		...
		const struct i2c_device_id *dev_id)
{
	...
	if (platform_data->regulator_en) {
		rmi4_data->regulator = regulator_get(&client->dev, "ldo19");
		...
		if(regulator_is_enabled(rmi4_data->regulator))
		{
			dev_err(&client->dev,"%s: Have enabled regulator already\n",__func__);
		}
		else
		{
			retval = regulator_enable(rmi4_data->regulator);
			if(retval < 0)
				dev_err(&client->dev,"%s:Failed to enable regulator\n",__func__);
		}
	}
	...
	printk("[TSP] start %s, Flip_status_tsp:%d, GPIO:%d\n", __FUNCTION__, Flip_status_tsp, gpio_get_value(GPIO_TSP_SEL));
#endif
	printk(KERN_INFO, "%s finish probe!", __func__);
	return retval;
