...
#include "zinitix_touch_infinite.h"
...
#define	SYSTEM_MAX_X_RESOLUTION	479
#define	SYSTEM_MAX_Y_RESOLUTION	799
...
//#define ZINITIX_DRIVER_NAME "msm_touchscreen"
	...
	if (!zinitix_bit_test(touch_dev->touch_info.status, BIT_PT_EXIST)) {
		for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
			...
			if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)) {
				...
				input_mt_report_slot_state(touch_dev->input_dev, 0, false);
...
#if defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_KYLE_PLUS) \
	|| defined(CONFIG_MACH_INFINITE_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
...
{
	printk(KERN_ERR"[TSP] %s : TA is %d, %d. \n", __func__, en, tsp_init_for_ta);
}
EXPORT_SYMBOL(touch_ta_noti);

#endif //  defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_ARUBA_OPEN)
...
#if	0 // jjlee_temp USE_TEST_RAW_TH_DATA_MODE
	if (ts_read_data(touch_dev->client,
...
#if	0 // jjlee_temp TOUCH_ONESHOT_UPGRADE
...
fail_init:
	...
	if (retry_cnt++ <= ZINITIX_INIT_RETRY_CNT) {
		...
		zinitix_debug_msg("retry to initiallize(retry cnt = %d)\r\n",
					retry_cnt);
#if 0 // jjlee_temp TOUCH_FORCE_UPGRADE
		if (retry_cnt > 15) {
	...
	for (i = 0; i < touch_dev->cap_info.multi_fingers; i++) {
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_EXIST)
			...
			if (w == 0)
				w = 5;
			input_mt_slot(touch_dev->input_dev, i);
			input_mt_report_slot_state(touch_dev->input_dev, 0, true);
...
//				input_report_abs(touch_dev->input_dev,	ABS_MT_POSITION_Y, y);
	...
	for (i = 0; i < touch_dev->cap_info.multi_fingers; i++)	{
		...
		if (zinitix_bit_test(sub_status, SUB_BIT_DOWN)
			...
			x = touch_dev->reported_touch_info.coord[i].x;
			y = touch_dev->reported_touch_info.coord[i].y;
			zinitix_debug_msg("finger [%02d] up %d,%d %d\r\n", i, x,y,0 );
...
#if 0 //jjlee_temp  (CONFIG_MACH_TREBON_HWREV == 0x0)
...
*******/
	...
	touch_dev->input_dev->evbit[0] = BIT_MASK(EV_ABS) | BIT_MASK(EV_KEY);
	set_bit(INPUT_PROP_DIRECT, touch_dev->input_dev->propbit);	//JB touch mode setting
	touch_dev->input_dev->keybit[BIT_WORD(KEY_MENU)] |= BIT_MASK(KEY_MENU);
	touch_dev->input_dev->keybit[BIT_WORD(KEY_BACK)] |= BIT_MASK(KEY_BACK);;
	input_mt_init_slots(touch_dev->input_dev, MAX_SUPPORTED_FINGER_NUM);
...
//	input_mt_init_slots(touch_dev->input_dev, 5);
	...
	input_set_abs_params(touch_dev->input_dev, ABS_MT_TRACKING_ID,	0, 4, 0, 0);
	...
	ret = request_threaded_irq(client->irq, NULL, ts_int_handler,	IRQF_TRIGGER_LOW | IRQF_ONESHOT, ZINITIX_DRIVER_NAME, touch_dev);
	if (ret) {
			printk(KERN_ERR "unable to register irq.(%s)\r\n",
				touch_dev->input_dev->name);
			goto err_request_irq;
