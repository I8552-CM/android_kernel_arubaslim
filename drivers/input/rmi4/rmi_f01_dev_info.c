...
static int rmi_f01_initialize(struct rmi_function_container *fc)
{
	...
	retval = rmi_write_block(rmi_dev, fc->fd.control_base_addr,
			...
			sizeof(data->device_control.ctrl0.reg));
	if (retval < 0) {
		dev_err(&fc->dev, "Failed to write MAIN TSP F01 control.\n");
		return retval;
	}
	...
	retval = rmi_write_block(rmi_dev, fc->fd.control_base_addr,
			...
			sizeof(data->device_control.ctrl0.reg));
	if (retval < 0) {
		dev_err(&fc->dev, "Failed to write SUB TSP F01 control.\n");
		return retval;
	}
	gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
	msleep(10);
...
static int rmi_f01_suspend(struct rmi_function_container *fc)
{
	...
	dev_info(&fc->dev, "F01 suspending...\n");
	if (data->suspended) {
		dev_info(&fc->dev, "Already suspended.\n");
		return 0;
	}
	...
	dev_info(&fc->dev, "Done with F01 suspend.\n");
