...
#if DUAL_TSP
#define BUTTON_LIMIT	3		// 2  W789 is use 3 touch key.  yuechun.yao
#else
#define BUTTON_LIMIT		2
#endif
...
int rmi_f1a_attention(struct rmi_function_container *fc, u8 *irq_bits)
{
	...
	for (button = 0; button < BUTTON_LIMIT ; button++) {
		...
		if(f1a->button_map[button])
		{
			printk("Button %d (code %d) -> %d.\n", button, f1a->button_map[button],	button_status);

			if(button_status == 0)
			{
				touchkey_raw[button] = 0;
			}
		}

	//	dev_dbg(&fc->dev, "%s: Button %d (code %d) -> %d.\n",__func__, button, f1a->button_map[button],	button_status);
