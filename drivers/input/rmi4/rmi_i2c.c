...
#endif
s16 touchkey_raw[2] = {0,0};
u8 gtouchkey_threshold;
...
#define NODE_NUM	187	/* 11x17 */
#define NODE_X_NUM 17//11
#define NODE_Y_NUM 11//17
...
{
	struct rmi_phys_device *phys = container_of(work,
				struct rmi_phys_device, work_dvfs_chg.work);
	int ret;
	mutex_lock(&phys->dvfs_lock);
	ret = set_freq_limit(DVFS_TOUCH_ID, 1008000);
	mutex_unlock(&phys->dvfs_lock);
...
{
	struct rmi_phys_device *phys= container_of(work,
				struct rmi_phys_device, work_dvfs_off.work);
	mutex_lock(&phys->dvfs_lock);
	set_freq_limit(DVFS_TOUCH_ID, -1);
	phys->dvfs_lock_status = false;
	mutex_unlock(&phys->dvfs_lock);

	printk(KERN_INFO "[TSP] DVFS Off!");
}

static void set_dvfs_lock(struct rmi_phys_device *phys, uint32_t on)
{
	int ret = 0;

	mutex_lock(&phys->dvfs_lock);
	if (on == 0) {
		if (phys->dvfs_lock_status) {
			schedule_delayed_work(&phys->work_dvfs_off,
				msecs_to_jiffies(TOUCH_BOOSTER_OFF_TIME));
			printk(KERN_INFO "[TSP] DVFS_touch_release");
		}
	} else if (on == 1) {
		cancel_delayed_work(&phys->work_dvfs_off);
		if (!phys->dvfs_lock_status) {
			ret = set_freq_limit(DVFS_TOUCH_ID, 1008000);
			if (ret < 0)
			...
			phys->dvfs_lock_status = true;
			printk(KERN_INFO "[TSP] DVFS On!");
		}
	} else if (on == 2) {
		cancel_delayed_work(&phys->work_dvfs_off);
		schedule_work(&phys->work_dvfs_off.work);
...
#if TOUCH_BOOSTER
	set_dvfs_lock(phys, !!touch_is_pressed);
#endif
	return IRQ_HANDLED;
}
...
static void get_fw_ver_bin(void *device_data)
{
	...
	if (hw_rev == 0x2200)
		snprintf(buff, sizeof(buff),"%c%c%.2d%.4d", FW_BIN_VERSION[0], FW_BIN_VERSION[1], FW_BIN_VERSION[2], FW_BIN_VERSION[3]);
...
static void get_fw_ver_ic(void *device_data)
{
	...
	snprintf(buff, sizeof(buff), "%c%c%.2d%.4d", FW_IC_VERSION[0], FW_IC_VERSION[1], FW_IC_VERSION[2], FW_IC_VERSION[3]);
...
static void get_threshold(void *device_data)
{
	...
	set_default_result(info);
	snprintf(buff, sizeof(buff), "%.3d", buf_temp[0]);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
...
static void get_tx_to_tx(void *device_data)
{
	...
	tx = info->cmd_param[0]; /* X */

	if (tx < 0 || tx > NODE_Y_NUM) {
		pr_err("tsp factory: param data is abnormal.\n");
		info->cmd_state = FAIL;
		return;
	}

	buf = info->node_data->tx_to_tx_data[tx];
	set_default_result(info);
	snprintf(buff, sizeof(buff), "%d", buf);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
...
static void get_tx_to_gnd(void *device_data)
{
	...
	buf = info->node_data->tx_to_gnd_data[tx];
	set_default_result(info);
	snprintf(buff, sizeof(buff), "%d", buf);
	set_cmd_result(info, buff, strnlen(buff, sizeof(buff)));
...
static void get_x_num(void *device_data)
{
	...
	//ver = info->fw_ic_ver;
	snprintf(buff, sizeof(buff), "%d", NODE_X_NUM-1);
...
static void run_tx_to_tx_read(void *device_data)
{
	...
	const int tx = NODE_Y_NUM;

	info->cmd_state = RUNNING;

	disable_irq(info->client->irq);

	if (!F54_TxToTest(info->client, info->node_data->tx_to_tx_data,
							TX_TO_TX_TEST_MODE)) {
		info->cmd_state = FAIL;
		return;
	}

	max_value = min_value = info->node_data->tx_to_tx_data[0];

	for (x = 0; x < tx; x++) {
		temp = info->node_data->tx_to_tx_data[x];
...
static void run_tx_to_gnd_read(void *device_data)
{
	...
	if (!F54_TxToTest(info->client, info->node_data->tx_to_gnd_data,
							TX_TO_GND_TEST_MODE)) {
		info->cmd_state = FAIL;
		return;
	}

	max_value = min_value = info->node_data->tx_to_gnd_data[0];

	for (x = 0; x < tx; x++) {
		temp = info->node_data->tx_to_gnd_data[x];
		max_value = max(max_value, temp);
		min_value = min(min_value, temp);
				  ...
				  const struct i2c_device_id *id)
{
	...
	if (error) {
		dev_err(&client->dev, "Failed to set SUB TSP page select to 0   error = %d.\n",error);
		goto err_data;
	}
	...
	error = rmi_set_page(rmi_phys, 0);
	if (error) {
		dev_err(&client->dev, "Failed to set MAIN TSP page select to 0   error = %d.\n",error);
		goto err_data;
	}
#else
	error = rmi_set_page(rmi_phys, 0);
	if (error) {
		dev_err(&client->dev, "Failed to set page select to 0   error = %d.\n",error);
		goto err_data;
	}
#endif
...
#if TOUCH_BOOSTER
		mutex_init(&rmi_phys->dvfs_lock);
		INIT_DELAYED_WORK(&rmi_phys->work_dvfs_off, set_dvfs_off);
		INIT_DELAYED_WORK(&rmi_phys->work_dvfs_chg, change_dvfs_lock);
		rmi_phys->dvfs_lock_status = false;
#endif    
...
///////////////////////////////////////test mode///////////////////////////////////////
	...
	if (IS_ERR(sec_touchscreen) || IS_ERR(sec_touchkey)) 
	{
...
#if DUAL_TSP
	/*check SUB TSP FW version and update */
	gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toSUB);
	msleep(10);
	set_fw_version(FW_KERNEL_VERSION, FW_DATE);
	fw_updater(client, "normal");  

	/* check  MAIN TSP FW version and update */
	gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
	msleep(10);
	set_fw_version(FW_KERNEL_VERSION, FW_DATE); 
	fw_updater(client, "normal");  
...
err_alloc_node_data_failed:
	...
	kfree(touch_dev->node_data->raw_cap_data);
	kfree(touch_dev->node_data->rx_to_rx_data);
	kfree(touch_dev->node_data->tx_to_tx_data);
	kfree(touch_dev->node_data->tx_to_gnd_data);
	kfree(touch_dev->node_data);
#endif	
...
{   
	...
	return sprintf(buf, "%c%c%.2d%.4d", FW_KERNEL_VERSION[0], FW_KERNEL_VERSION[1], FW_KERNEL_VERSION[2], FW_KERNEL_VERSION[3]);
}
...
{   
	...
	printk("[TSP] %s, %s !!! \n",__func__,FW_IC_VERSION);
    
	return sprintf(buf,  "%c%c%.2d%.4d", FW_IC_VERSION[0], FW_IC_VERSION[1], FW_IC_VERSION[2], FW_IC_VERSION[3]);
}
...
static ssize_t synaptics_touchkey_threshold_show(struct device *dev, struct device_attribute *attr, char *buf)
{
	...
	readTouchKeyThreshold(gb_client, &command);

	enable_irq(info->client->irq);

	gtouchkey_threshold = command;

//	printk("TSP Threshold Value : %d\n",  command);
