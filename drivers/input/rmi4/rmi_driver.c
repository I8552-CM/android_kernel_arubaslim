			      ...
			      struct rmi_function_handler *fh)
{
	...
	if (!data)
		return;
	/* We need to lock here to avoid race with interrupt handling. */
...
static int process_interrupt_requests(struct rmi_device *rmi_dev)
{
	...
	if (data->f01_container->irq_mask && data->f01_container->fh->attention) {
 	...
 	/* We need to lock here to avoid race with function container lists. */
	mutex_lock(&data->irq_mutex);
	...
	for (page = 0; (page <= RMI4_MAX_PAGE) && !done; page++) {
		...
		for (i = pdt_start; i >= pdt_end; i -= sizeof(pdt_entry)) {
			...
			if (pdt_entry.function_number == 0x01) {
				...
				dev_info(dev, "Post rest delay start.\n");
				mdelay(pdata->reset_delay_ms);
				dev_info(dev, "Delay end.\n");
				dev_info(dev, "Delay should have been %d ms.\n",
...
static int rmi_driver_probe(struct rmi_device *rmi_dev)
{
	...
	msleep(10);
	retval = do_initial_reset(rmi_dev);
	printk("[TSP]%d:finish do_initial_reset,Main TSP used",__func__);
#else
	retval = do_initial_reset(rmi_dev);
#endif
	if (retval)
...
#endif
#if DUAL_TSP
	Tsp_probe_passed = 1;	//yuechun.yao	
#endif
	return 0;
...
static int standard_suspend(struct rmi_device *rmi_dev)
{
	...
	dev_info(&rmi_dev->dev, "Suspending...\n");
	...
	dev_info(&rmi_dev->dev, "Suspend completed.\n");
...
static int standard_resume(struct rmi_device *rmi_dev)
{
	...
	dev_info(&rmi_dev->dev, "Resume completed.\n");
...
static void standard_early_suspend(struct early_suspend *h)
{
	...
	data = rmi_get_driverdata(rmi_dev);
	dev_info(&rmi_dev->dev, "Early suspend.\n");

	mutex_lock(&data->suspend_mutex);
	if (data->early_suspended) {
		dev_info(&rmi_dev->dev, "Already in early suspend.\n");
		goto exit;
	}
	...
	dev_info(&rmi_dev->dev, "Early suspend completed.\n");
exit:
	mutex_unlock(&data->suspend_mutex);
}
...
static void standard_late_resume(struct early_suspend *h)
{
	...
	data = rmi_get_driverdata(rmi_dev);
	dev_info(&rmi_dev->dev, "Late resume.\n");

	mutex_lock(&data->suspend_mutex);
	if (!data->early_suspended) {
		dev_info(&rmi_dev->dev, "Not in early suspend.\n");
		goto exit;
	}
	...
	dev_info(&rmi_dev->dev, "Late resume completed.\n");
...
static int rmi_driver_suspend(struct device *dev) {
	...
	dev_info(&rmi_dev->dev, "Suspend is a no-op.");
	return 0;
}
...
static int rmi_driver_resume(struct device *dev) {
	...
	struct rmi_device *rmi_dev = to_rmi_device(dev);
	dev_info(&rmi_dev->dev, "Resume is a no-op.");
	return 0;
}
...
void samsung_switching_tsp(int flip)
{
	...
	if (Flip_status_tsp == FLIP_NOTINIT) {
		...
		gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
		printk("[TSP] start %s, Flip_status_tsp:%d\n", __FUNCTION__, Flip_status_tsp);
		return;
	}
	...
	if (Flip_status_tsp == FLIP_OPEN) {
		...
		standard_early_suspend(&(copy_data->early_suspend_handler));
		//resume the main screen
		gpio_set_value(GPIO_TSP_SEL, TSP_SEL_toMAIN);
		printk("[TSP]resume the main screen, gpio_tsp_sel = %d\n",gpio_get_value(GPIO_TSP_SEL));
		standard_late_resume(&(copy_data->early_suspend_handler));
