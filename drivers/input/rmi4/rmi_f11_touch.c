...
//		add for debug		
		if (data->fg_state == 1){
			pr_info("[TSP] finger up, x=%d, y=%d, z=%d\n", x, y, z);
                 touch_is_pressed=0;
			data->fg_state = 0;
		}
/////////////////////////////////////////////////////		
...
//		add for debug				
		if (data->fg_state == 0){
			pr_info("[TSP] finger down, x=%d, y=%d, z=%d\n", x, y, z);
                 touch_is_pressed=1;
			data->fg_state = 1;
		}
/////////////////////////////////////////////////////				
	...
	if (f11->type_b) {
		...
		input_mt_report_slot_state(sensor->input,  get_tool_type(sensor, finger_state),  finger_state);
	}
	else
		input_report_abs(sensor->input, ABS_MT_TOOL_TYPE, get_tool_type(sensor, finger_state));
#else
