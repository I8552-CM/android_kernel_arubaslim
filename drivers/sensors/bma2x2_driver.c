...
#if 0//def CONFIG_BMA_USE_PLATFORM_DATA
		...
		struct bma2x2_data *client_data)
{
	...
	{
		client_data->pdata->place = 2;
	}
#elif defined(CONFIG_MACH_BAFFIN_DUOS_CTC)
	if ((board_hw_revision >= 3) && (board_hw_revision != 6))
	{
		client_data->pdata->place = 5;
	}
#elif defined(CONFIG_MACH_DELOS_DUOS_CTC)
	if (board_hw_revision >= 6)
	{
		client_data->pdata->place = 7;
	}
#elif defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
	client_data->pdata->place = 1;
#endif
		...
		int addr_len, unsigned char *rxData, int length)
{
	...
	do {
		err = i2c_transfer(bma2x2_data->bma2x2_client->adapter, msgs, 2);
	} while ((err != 2) && (++tries < I2C_RETRIES));

	if (err != 2) {
		dev_err(&bma2x2_data->bma2x2_client->adapter->dev, "[BMA] read transfer error\n");
		err = -EIO;
	} else {
		err = 0;
	}
		...
		int addr_len, unsigned char *txData, int length)
{
	...
	err = i2c_transfer(bma2x2_data->bma2x2_client->adapter, msg, 1);
	if (err != 1) {
		dev_err(&bma2x2_data->bma2x2_client->adapter->dev,
		"[BMA] i2c_transfer() write error: slave_addr=%02x, reg_addr=%02x, err=%d\n",
			chip, adr, err);
		return err;
	}
...
static int bma255_init_library(void)
{
...
static int bma2x2_set_mode(struct i2c_client *client, unsigned char Mode)
{
	...
	pr_info("%s: %d\n reactive_enable = %d, reactive_state =%d\n", __func__, Mode,
...
#ifdef CONFIG_INPUT_BMA2x2_ACC_ALERT_INT
	mutex_lock(&bma2x2->data_mutex);
	pr_info("%s: %d \n reactive_enable = %d, reactive_enable =%d\n", __func__, 
	enable, atomic_read(&bma2x2->reactive_enable), atomic_read(&bma2x2->reactive_state));
#endif
		...
		struct device_attribute *attr, char *buf)
{
	...
	printk(KERN_INFO "bma2x2_calibration_show");
		...
		const char *buf, size_t count)
{
	...
	if(data){
#if defined (CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_DELOS_OPEN) || defined(CONFIG_MACH_DELOS_CTC) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
...
static struct device_attribute *bma2x2_attr[] = {
	&dev_attr_range,
	&dev_attr_bandwidth,
	&dev_attr_mode,
	&dev_attr_raw_data,
...
static int bma2x2_setup_irq(struct bma2x2_data *bma2x2)
{
	...
	printk(KERN_INFO "%s\n",__func__);
	rc = gpio_request(pdata->p_out, "gpio_bma2x2_int");
	if (rc < 0) {
...
//	bma2x2_set_Int_Enable(client, 11, 1);
	...
	if (NULL != client->dev.platform_data) {
		data->pdata = kzalloc(sizeof(*data->pdata),
				GFP_KERNEL);
	...
	err = sensors_register(&bma2x2_device,
			data, bma2x2_attr,"accelerometer_sensor");
	if (err < 0) {
...
static void bma2x2_early_suspend(struct early_suspend *h)
{
 ...
 && !defined(CONFIG_MACH_BAFFIN_DUOS_CTC) && !defined(CONFIG_MACH_INFINITE_DUOS_CTC) && !defined(CONFIG_MACH_DELOS_DUOS_CTC) && !defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
