...
turn_off:
	if (err < 0)
		i2c_smbus_write_byte_data(k3g_data->client, CTRL_REG1, 0x00);
unlock:
...
static DEVICE_ATTR(power_on,  0664, k3g_power_on, NULL);
static DEVICE_ATTR(power_off, 0664, k3g_power_off, NULL);
static DEVICE_ATTR(temperature,  0664, k3g_get_temp, NULL);
static DEVICE_ATTR(selftest,  0664, k3g_self_test, NULL);
static DEVICE_ATTR(selftest_dps, 0664, k3g_selftest_dps_show, k3g_selftest_dps_store);
...
*/
	...
	if (data->client->irq >= 0) { /* interrupt */
		...
		err = request_threaded_irq(data->client->irq, NULL, k3g_interrupt_thread, IRQF_TRIGGER_HIGH | 
		IRQF_ONESHOT, "k3g", data);
		if (err < 0) {
	...
	if (device_create_file(data->dev, &dev_attr_power_on) < 0) {
		pr_err("%s: Failed to create device file(%s)!\n", __func__, dev_attr_power_on.attr.name);
		goto device_create_file1;
	}

	if (device_create_file(data->dev, &dev_attr_temperature) < 0) {
		pr_err("%s: Failed to create device file(%s)!\n", __func__, dev_attr_temperature.attr.name);
		goto device_create_file2;
	}

	if (device_create_file(data->dev, &dev_attr_selftest) < 0) {
		pr_err("%s: Failed to create device file(%s)!\n", __func__, dev_attr_selftest.attr.name);
		goto device_create_file3;
	}

	if (device_create_file(data->dev, &dev_attr_selftest_dps) < 0) {
		pr_err("%s: Failed to create device file(%s)!\n", __func__, dev_attr_selftest_dps.attr.name);
		goto device_create_file4;
	}

	if (device_create_file(data->dev, &dev_attr_power_off) < 0) {
		pr_err("%s: Failed to create device file(%s)!\n", __func__, dev_attr_power_off.attr.name);
		goto device_create_file5;
	}
	
	dev_set_drvdata(data->dev, data);

	/*** set sysfs for factory test ***/
	data->gyro_dev = device_create(sensors_class,
					    NULL, 0, NULL, "gyro_sensor");
	if (IS_ERR(data->gyro_dev)) {
		pr_err("%s: could not create proximity_dev\n", __func__);
		goto err_sys_device_create;
	}
	
	/* power on */
	if (device_create_file(data->gyro_dev,
		&dev_attr_power_on) < 0) {
		pr_err("%s: could not create device file(%s)!\n", __func__,
		       dev_attr_power_on.attr.name);
		goto err_sys_device_create_file1;
	}
	/* power off */
	if (device_create_file(data->gyro_dev,
		&dev_attr_power_off) < 0) {
		pr_err("%s: could not create device file(%s)!\n", __func__,
		       dev_attr_power_off.attr.name);
		goto err_sys_device_create_file2;
	}
	/* selftest */
	if (device_create_file(data->gyro_dev,
		&dev_attr_selftest) < 0) {
		...
		goto err_sys_device_create_file3;
	}
	/* selftest_dps */
	if (device_create_file(data->gyro_dev,
		&dev_attr_selftest_dps) < 0) {
		pr_err("%s: could not create device file(%s)!\n", __func__,
		       dev_attr_selftest_dps.attr.name);
		goto err_sys_device_create_file4;
	}
	/* temprature */
	if (device_create_file(data->gyro_dev,
		&dev_attr_temperature) < 0) {
		pr_err("%s: could not create device file(%s)!\n", __func__,
		       dev_attr_temperature.attr.name);
		goto err_sys_device_create_file5;
	}
	dev_set_drvdata(data->gyro_dev, data);

	return 0;

err_sys_device_create_file5:
	device_remove_file(data->gyro_dev, &dev_attr_selftest_dps);
err_sys_device_create_file4:
	device_remove_file(data->gyro_dev, &dev_attr_selftest);
err_sys_device_create_file3:
	device_remove_file(data->gyro_dev, &dev_attr_power_off);
err_sys_device_create_file2:
	device_remove_file(data->gyro_dev, &dev_attr_power_on);
err_sys_device_create_file1:
	device_destroy(sensors_class, 0);
err_sys_device_create:
device_create_file5:
...
static int k3g_remove(struct i2c_client *client)
{
	...
	if (sensors_class != NULL) {
		device_remove_file(k3g_data->gyro_dev, &dev_attr_temperature);
		device_remove_file(k3g_data->gyro_dev, &dev_attr_power_on);
		device_remove_file(k3g_data->gyro_dev, &dev_attr_power_off);
		device_remove_file(k3g_data->gyro_dev, &dev_attr_selftest);
		device_remove_file(k3g_data->gyro_dev, &dev_attr_selftest_dps);
		device_destroy(sensors_class, 0);
	}

	if (k3g_data->k3g_gyro_dev_class != NULL) {
		device_remove_file(k3g_data->dev, &dev_attr_temperature);
		device_remove_file(k3g_data->dev, &dev_attr_power_on);
		device_remove_file(k3g_data->dev, &dev_attr_power_off);
		device_remove_file(k3g_data->dev, &dev_attr_selftest);
		device_remove_file(k3g_data->dev, &dev_attr_selftest_dps);
