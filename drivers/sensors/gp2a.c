...
defined(CONFIG_MACH_KYLEPLUS_CTC)||defined(CONFIG_MACH_KYLEPLUS_OPEN) || defined(CONFIG_MACH_INFINITE_DUOS_CTC) || defined(CONFIG_MACH_DELOS_DUOS_CTC)|| \
defined(CONFIG_MACH_ROY) || defined(CONFIG_MACH_DELOS_OPEN) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
...
#if defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_KYLEPLUS_OPEN) || \
defined(CONFIG_MACH_ARUBA_CTC) && !defined(CONFIG_MACH_DELOS_OPEN)
/* B1.5 mode */
...
#if defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_KYLEPLUS_OPEN) ||\
defined(CONFIG_MACH_ARUBA_CTC) && !defined(CONFIG_MACH_DELOS_OPEN)
/* B1.5 mode */
...
#elif defined(CONFIG_MACH_ROY)
/* Roy mode (Modified B1.5 mode) : value will be changed */
#define PROX_NONDETECT_MODE1	0x41
#define PROX_DETECT_MODE1		0x2E
#define PROX_NONDETECT_MODE2	0x4E
#define PROX_DETECT_MODE2		0x2B
...
#endif //GP2AP002X_PROXIMITY_OFFSET
			  ...
			  const struct i2c_device_id *id)
{
	...
	/* power on gp2a */
	//pdata->power(true);no need to enable &disable the power in the proxi sensor probe
	...
	//pdata->power(false);no need to enable &disable the power in the proxi sensor probe
	goto done;
...
err_input_register_device_proximity:
	pr_info("err_input_register_device_proximity\n");
...
{
#if !defined(CONFIG_MACH_KYLE) && !defined(CONFIG_MACH_ARUBA_OPEN) && !defined (CONFIG_MACH_ARUBASLIM_OPEN)
...
{
	pr_info("[TMP] %s, %d\n", __func__, __LINE__);
#if defined(CONFIG_MACH_ROY) && !defined(CONFIG_MACH_ROY_DTV)
	if (board_hw_revision < 4)
	{
		pr_err("%s: hw_revision(%d) : No gp2a\n", __func__, board_hw_revision);
		return -1;
	}
#endif
