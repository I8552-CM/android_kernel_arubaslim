...
static ssize_t bmm_show_value_raw(struct device *dev,
		struct device_attribute *attr, char *buf)
{
#if defined (CONFIG_MACH_ARUBASLIM_OPEN) || defined(CONFIG_MACH_DELOS_DUOS_CTC) || defined(CONFIG_MACH_DELOS_OPEN) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
	struct bmm050_mdata_s32 value;
	int count;
		...
		const char *buf, size_t count)
{
	...
	if (BMM_SELF_TEST == data) {
		...
		mdelay(3);
		BMM_CALL_API(set_selftest)(1);
		mdelay(3);
		BMM_CALL_API(get_self_test_XYZ)(&dummy);
...
static struct device_attribute *bmm_attr[] = {
	&dev_attr_chip_id,
	&dev_attr_op_mode,
	&dev_attr_odr,
	&dev_attr_rept_xy,
...
static int bmm_probe(struct i2c_client *client, const struct i2c_device_id *id)
{
	...
	mdelay(30); // add delay time to prevent i2c fail
