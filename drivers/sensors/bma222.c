...
{
	pr_info("[ACC]jaehee1 probe_done: %d \n", probe_done);
	...
	const struct i2c_device_id *id)
{
	...
	if (!bma222->input_dev) {
		...
		err = -ENOMEM;
		goto exit_bma222_input_allocate_device;
	}
	...
	err = sensors_register(&bma222_device,
		NULL, bma222_attrs,
		"accelerometer_sensor");
	if (err < 0) {
		pr_info("%s: could not sensors_register\n", __func__);
		goto exit_bma222_sensors_register;
	}
	...
	pr_info("[ACC]jaehee probe_done: %d \n", probe_done);

	return 0;
exit_bma222_sensors_register:
	sysfs_remove_group(&bma222->input_dev->dev.kobj,
		&bma222_attribute_group);
exit_bma222_device:
...
exit_bma222_input_allocate_device:
