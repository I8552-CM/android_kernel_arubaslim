...
#ifndef CONFIG_SERIAL_MSM_RX_WAKEUP
#define CONFIG_SERIAL_MSM_RX_WAKEUP
#endif
...
static irqreturn_t msm_rx_irq(int irq, void *dev_id)
{
	...
	printk(KERN_ERR "(SJINU %s) enter interrupt service routine \n", __func__);
	...
	if (wakeup) {
		/* let it self expire */
		wake_lock_timeout(&msm_port->wakeup.wake_lock,
...
#if 0 //def CONFIG_SERIAL_MSM_RX_WAKEUP
...
static int __init msm_serial_probe(struct platform_device *pdev)
{
	...
	if (pdata == NULL)
	{
		printk("(SJINU %s) platform data=NULL \n", __func__);				
		msm_port->wakeup.irq = -1;
	}
	else {
		msm_port->wakeup.rx_gpio = pdata->wakeup_irq;
		msm_port->wakeup.irq = MSM_GPIO_TO_INT(pdata->wakeup_irq);
		msm_port->wakeup.ignore = 1;
		...
		{
			printk("(SJINU %s) FAIL msm_port->wakeup.irq <= 0 \n", __func__);		
			return -EINVAL;
		}

		/* Create a wake_lock to prevent system from suspend */
		wake_lock_init(&msm_port->wakeup.wake_lock,
			WAKE_LOCK_SUSPEND, "msm_serial");

		ret = request_irq(msm_port->wakeup.irq, msm_rx_irq,
			IRQF_TRIGGER_FALLING, "msm_serial_wakeup", msm_port);

		if (unlikely(ret)) {
			pr_err("(SJINU %s) failed to request wakeup_irq\n", __func__);
			return ret;
		}
...
/* When using UART1, to save sleep current, we should deinit uart1 clock */
		 ...
		 * */
		if (msm_port->wakeup.irq > 0) {
			printk(KERN_ERR "(SJINU %s) enabling wakeup irq\n", __func__);
			enable_irq_wake(msm_port->wakeup.irq);
			enable_irq(msm_port->wakeup.irq);
			msm_port->wakeup.wakeup_set = 1;
		}
	}
...
static int msm_serial_resume(struct device *dev)
{
	...
	if (uart_connecting || msm_port->wakeup.wakeup_set) {
		/* disable wakeup_irq */
		if (msm_port->wakeup.irq > 0) {
			printk(KERN_ERR "%s disbling wakeup irq\n",
							__func__);
			if (!msm_port->wakeup.wakeup_set)
				return 0;
