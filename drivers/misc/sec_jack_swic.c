...
struct sec_jack_info {
	...
	struct input_handler handler;  //?? what's this?
	struct input_handle handle;  //?? what's this?
	int dev_id;
	int pressed;
	int pressed_code;
...
struct switch_dev swic_sendend = {
		.name = "send-end",
};
...
void jack_status_change(int status)
{
	...
	if (status) {
...
#if defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
		gpio_set_value(MIC_SELET_GPIO,0);
#endif
	} else {
		...
		if (send_end_pressed == 1)	{
			...
			switch_set_state(&swic_sendend, 0);
			send_end_pressed = 0;
		}

		hi->pdata->set_micbias_state(false); 
#if defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
		gpio_set_value(MIC_SELET_GPIO,1);
#endif
	}
	switch_set_state(&switch_jack_detection, jack_type);
	hi->cur_jack_type = jack_type;
...
void earkey_status_change(int pressed, int kcode)
{
	...
	pr_info("%s kcode=%d, pressed = %d\n", __func__, kcode, pressed);
	if (pressed) {

		input_report_key(hi->input_dev, hi->pressed_code, 1);
		switch_set_state(&swic_sendend, 1);
		input_sync(hi->input_dev);
		send_end_pressed = 1;
		msleep(50);
	} else {
...
static int sec_jack_probe(struct platform_device *pdev)
{
	...
	pr_info("%s : Swic Registering jack driver\n", __func__);
	if (!pdata) {
	...
	ret = switch_dev_register(&swic_sendend);
	if (ret < 0) {
...
err_create_wq_failed:
	...
	switch_dev_unregister(&swic_sendend);
