...
#if defined(CONFIG_CDMA_MODEM_MDM6600) || defined(CONFIG_GSM_MODEM_ESC6270) || defined(CONFIG_GSM_MODEM_SPRD6500)
enum qc_dload_tag {
	QC_DLOAD_TAG_NONE = 0,
	QC_DLOAD_TAG_BIN,
	QC_DLOAD_TAG_NV,
			...
			struct dpram_udl_param *param)
{
	...
	if (param->size <= dpld->dpctl->max_boot_frame_size) {
		iowrite16(PLD_ADDR_MASK(&bt_map->buff[0]),
					dpld->address_buffer);
		_memcpy_write(dpld->dp_base, param->addr, param->size);
			...
			enum qc_dload_tag tag)
{
	...
	mif_info("param_addr[0x%x], param.size[0x%x]\n", (u32)param.addr, param.size);
	//mif_print_data(param.addr, param.size);
...
static int qc_boot_start(struct dpram_link_device *dpld)
{
	...
	mif_err("Send interrypt GIL 0x4567%d\n", count);
	mask = 0x4567;
...
static void qc_crash_log(struct dpram_link_device *dpld)
{
	...
	_memcpy_read(buf, data, (sizeof(buf) - 1));
			...
			struct dpram_udl_param *param)
{
	...
	iowrite16(PLD_ADDR_MASK(&bt_map->buff[0]),
				dpld->address_buffer);
	_memcpy_read(param->addr, dpld->dp_base, param->size);
...
static int qc_check_cp_bin(struct dpram_link_device *dpld)
{
	...
	if (board_hw_revision >= 4)	{
		gpio_set_value(GPIO_UART1_SEL, retval);	// UART1_SEL
	}
#endif
	return retval;
}

static int qc_updated_cp_bin(struct dpram_link_device *dpld)
{
	/* don't need to download : 0 */
...
#if defined(CONFIG_MACH_DELOS_DUOS_CTC)
	if (board_hw_revision >= 4)	{
		gpio_set_value(GPIO_UART1_SEL, 0);	// UART1_SEL
	}
#endif
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_DPRAM_CHECK_CP_BIN:
		err = qc_check_cp_bin(dpld);
		mif_info("%s: check_cp_bin [%d]\n", ld->name, err);
		break;

	case IOCTL_DPRAM_UPDATED_CP_BIN:
		err = qc_updated_cp_bin(dpld);
		mif_info("%s: check_cp_bin [%d]\n", ld->name, err);
		break;
...
struct dpram_ext_op *pld_get_ext_op(enum modem_t modem)
{
