...
static void qc_init_boot_map(struct dpram_link_device *dpld)
{
	...
	bt_map->buff = dpld->base;
	bt_map->frame_size = (u16 *)(dpld->base + dpctl->boot_size_offset);
	bt_map->tag = (u16 *)(dpld->base + dpctl->boot_tag_offset);
	bt_map->count = (u16 *)(dpld->base + dpctl->boot_count_offset);
		...
		unsigned int cmd, unsigned long arg)
{
	...
	case IOCTL_DPRAM_PHONEIMG_LOAD:
		mif_err("IOCTL_DPRAM_PHONEIMG_LOAD %s\n", ld->name);
	...
	case IOCTL_DPRAM_NVDATA_LOAD:
		mif_err("IOCTL_DPRAM_NVDATA_LOAD %s\n", ld->name);
