...
static inline bool circ_valid(u32 size, u32 in, u32 out);
...
static inline void clear_intr(struct dpram_link_device *dpld)
{
	if (likely(dpld->ext_op && dpld->ext_op->clear_intr))
		dpld->ext_op->clear_intr(dpld);
	else	{
		iowrite16(0, dpld->mbx2ap);
	}
}
...
static inline void send_intr(struct dpram_link_device *dpld, u16 mask)
{
	...
	//writel_relaxed(1, MSM_A2M_INT(3));

	wmb();
	__raw_writel(1, MSM_A2M_INT(3));
}
...
static void dpram_netif_stop(struct dpram_link_device *dpld)
{
	struct link_device *ld = &dpld->ld;
	struct io_device *iod =NULL;
	
	if (ld->ipc_version < SIPC_VER_50)
		iod = link_get_iod_with_channel(ld, 0x20 | PS_DATA_CH_0);
	else
...
static void dpram_netif_wake(struct dpram_link_device *dpld)
{
	struct link_device *ld = &dpld->ld;
	struct io_device *iod =NULL;
...
static void dpram_write(struct dpram_link_device *dpld, int dev, u32 qsize,
...
static int dpram_ipc_tx(struct dpram_link_device *dpld, int dev)
{
	...
	while (1) {
		...
		dpram_write(dpld, dev, qsize, in, out, skb);
		copied += skb->len;
...
static int dpram_wait_for_res_ack(struct dpram_link_device *dpld, int dev)
{
	...
	if (ret == 0) {
		mif_err("%s: %s: TIMEOUT\n", ld->name, get_dev_name(dev));
		queue_delayed_work(ld->tx_wq, ld->tx_dwork[dev], REQ_ACK_DELAY);
	} else if (ret < 0) {
...
static void dpram_read(struct dpram_link_device *dpld, int dev, u8 *dst,
...
static int dpram_recv_ipc_with_rxb(struct dpram_link_device *dpld, int dev)
{
	...
	dpram_read(dpld, dev, rxb_put(rxb, rcvd), src, out, rcvd, qsize);
...
static int dpram_recv_ipc_with_skb(struct dpram_link_device *dpld, int dev)
{
	...
	while (rest > 0) {
		...
		/* Read data from each DPRAM buffer */
		dst = skb_put(skb, tot);
		dpram_read(dpld, dev, dst, src, out, tot, qsize);
		skb_trim(skb, len);
			...
			struct io_device *iod, struct sk_buff *skb)
{
	...
	if (likely(dpram_can_sleep(dpld))) {
		if (dpram_wake_up(dpld) < 0) {
			trigger_force_cp_crash(dpld);
			return;
		}
...
struct link_device *dpram_create_link_device(struct platform_device *pdev)
{
	...
	*/
	if (!dpctl->dp_base) {
		...
		dpctl->dp_base = (struct u8 __iomem *)
			(smem_alloc2(res->start, res->end));

		if (!dpctl->dp_base) {
			mif_err("%s: ERR! smem_alloc2 for BASE fail\n",
				ld->name);
			goto err;
		}

		dpctl->dp_size = res->end;
	}
	...
	for (i = 0; i < ld->max_ipc_dev; i++) {
		...
		if (!dpld->rxbq[i].rxb) {
			mif_err("%s: ERR! %s rxbq_create_pool fail!! [%d][%d]\n",
	...
	*/
	...
	if (!res) {
		mif_err("%s: Failed to get SMD arg\n", ld->name);
		goto err;
	}
	...
	if (dpctl->dpram_irq) {
		...
		res = platform_get_resource_byname(pdev, IORESOURCE_IRQ, "smd_irq_info");
		dpld->irq = res->start;
		if (dpld->irq < 0) {
	...
	*/
	...
	clear_intr(dpld);
	mif_info("rcvd[%d], mbx2ap[0x%x], mbx2cp [0x%x]\n", recv_intr(dpld), (u32)dpld->mbx2ap, (u32)dpld->mbx2cp);
...
/*
	ret = register_isr(dpld->irq, dpld->irq_handler, dpld->irq_flags,
				dpld->irq_name, dpld);
*/
	ret = request_irq(MSM8625_INT_A9_M2A_3,
		dpld->irq_handler, IRQF_TRIGGER_RISING, dpld->irq_name, dpld);

	if (ret)	{
		mif_err("%s: Failed to register SMD ISR\n", ld->name);
		goto err;
	}
