...
void _memcpy_read(void *p_dest, const void *p_src, int size);
void _memcpy_write(void *p_dest, const void *p_src, int size);
		...
		u32 qsize, u32 *in, u32 *out)
{
	...
	do {
		...
		space = (head < tail) ? (tail - head - 1) :
			(qsize + tail - head - 1);
		mif_info("%s: %s_TXQ qsize[%u] in[%u] out[%u] space[%u]\n",
		...
		u32 qsize, u32 in, u32 out, struct sk_buff *skb)
{
	...
	if (in < out) {
		...
		_memcpy_write(dpld->dp_base, src, len);
	} else {
		/* ------ out +++++++++++ in ------------ */
		u32 space = qsize - in;
		...
		/* 1) in -> buffer end */
		iowrite16(PLD_ADDR_MASK(&(buff+in)[0]), dpld->address_buffer);
		_memcpy_write(dpld->dp_base, src, ((len > space) ? space : len));

		if (len > space)	{
			iowrite16(PLD_ADDR_MASK(&buff[0]),
						dpld->address_buffer);
			_memcpy_write(dpld->dp_base, (src+space), (len-space));
		}
	}
	...
	u8 __iomem *src, u32 out, u32 len, u32 qsize)
{
	...
	if ((out + len) <= qsize) {
		...
		_memcpy_read(dst, dpld->dp_base, len);
	} else {
		/*       (in) ----------- (out)   */
		/* 00 7e      -----------   7f 00 */
		...
		/* 1) out -> buffer end */
		iowrite16(PLD_ADDR_MASK(&(src+out)[0]), dpld->address_buffer);
		_memcpy_read(dst, dpld->dp_base, len1);

		/* 2) buffer start -> in */
		dst += len1;
		iowrite16(PLD_ADDR_MASK(&src[0]), dpld->address_buffer);
		_memcpy_read(dst, dpld->dp_base, (len - len1));
	}
		...
		struct sk_buff *skb)
{
	...
	case IPC_RFS:
		if (likely(ld->mode == LINK_MODE_IPC)) {
			...
			mif_info("%s: ld->mode[%d] != LINK_MODE_IPC\n", ld->name, ld->mode);
			dev_kfree_skb_any(skb);
		}
		return len;
...
u8 pld_write_buffer[0x1000];
u8 pld_read_buffer[0x1000];
static void pld_loopback_test(struct dpram_link_device *dpld)
{
	int i=0, j=0;
	u32 address=0;
	u32 copy_size = 36;
	...
	//pld_write_buffer = kmalloc(0x100, GFP_KERNEL);
	//pld_read_buffer = kmalloc(0x100, GFP_KERNEL);

	//set_magic(dpld, 0xAB);
	//set_access(dpld, 0xCD);
	//set_tx_head(dpld, 0, 0x12);
	//set_tx_head(dpld, 1, 0x34);

	memset(pld_write_buffer, 0x0, 0x1000);
	for(i=0 ; i < 0x1000 ; i++)	{
		pld_write_buffer[i] = (u8)i;
 	}
/*
	for(i = 0 ; i < 0x10 ; i++)	{
		address = 0x2000 + i * 0x100;
		iowrite16(address, dpld->address_buffer);

		mif_info("write adress:[0x%x]\n", address);
		memcpy(dpld->dp_base, pld_write_buffer, 0x100);
	}

	for(i = 0 ; i < 0x10 ; i++)	{
		address = 0x3000 + i * 0x100;

		mif_info("read adress:[0x%x]\n", address);
		memset(pld_read_buffer, 0x0, 0x100);
		iowrite16(address, dpld->address_buffer);
		memcpy(pld_read_buffer, dpld->dp_base, 0x100);

		if(memcmp(pld_write_buffer, pld_read_buffer, 0x100))	{
			mif_info("pld_loopback_test error!! [%d]\n", i);
			mif_print_data(pld_read_buffer, 0x100);

			//kfree(pld_write_buffer);
			//kfree(pld_read_buffer);
			return;
		}
	}
*/
...
/*
	i=0;
	iowrite16(0x2000, dpld->address_buffer);
	while (i++ < 0x400)	{
		iowrite8(pld_write_buffer[i], dpld->dp_base);
	}
*/

	iowrite16(0x2000, dpld->address_buffer);
	_memcpy_write(dpld->dp_base, pld_write_buffer, copy_size);
	//mif_print_data(pld_write_buffer, copy_size);

	msleep(20);

	memset(pld_read_buffer, 0x0, 0x1000);
/*
	i=0;
	iowrite16(0x2000, dpld->address_buffer);
	while (i++ < 0x400)	{
		pld_read_buffer[i] = ioread8(dpld->dp_base);
		//(u8)pld_read_buffer[i] = (u8)*(dpld->dp_base);
	}
*/

	iowrite16(0x2000, dpld->address_buffer);
	iowrite16(0x2000, dpld->address_buffer);
	memcpy(pld_read_buffer, dpld->dp_base, copy_size);

	mif_print_data(pld_read_buffer, copy_size);
	if(memcmp(pld_write_buffer, pld_read_buffer, copy_size))	{
		mif_info("pld_loopback_test error!! [%d]\n", i);
	}

	iowrite16(0x2000, dpld->address_buffer);
	_memcpy_write(dpld->dp_base, pld_write_buffer, copy_size2);
	//mif_print_data(pld_write_buffer, copy_size2);
	...
	iowrite16(0x2000, dpld->address_buffer);
	iowrite16(0x2000, dpld->address_buffer);
	memcpy(pld_read_buffer, dpld->dp_base, copy_size2);

	mif_print_data(pld_read_buffer, copy_size2);
	if(memcmp(pld_write_buffer, pld_read_buffer, copy_size2))	{
		mif_info("pld_loopback_test error!! [%d]\n", i);
		return;
	}
	...
	mif_info("pld_loopback_test() ended properly!!\n");
...
struct link_device *pld_create_link_device(struct platform_device *pdev)
{
	...
	dpld->ext_op = pld_get_ext_op(mdm_data->modem_type);
