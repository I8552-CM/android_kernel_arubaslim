...
struct switch_dev switch_sendend_key = {
	.name = "send-end",
}; 
...
static void set_send_key_state(struct sec_jack_info *hi, int state)
{
	...
	input_sync(hi->input);
	switch_set_state(&switch_sendend_key, state);
...
static irqreturn_t sec_jack_detect_irq_thread(int irq, void *dev_id)
{
	...
	pr_info(MODULE_NAME "%s : jack is detected!!\n", __func__);
	...
	/* G plus L Detection */
	if (!pdata->get_l_jack_state())
		return;

	pr_info("[IJ] G = %d, L = %d", pdata->get_det_jack_state(), \
...
#ifndef FEATURE_HSSD
	ret = switch_dev_register(&switch_sendend_key);
	if (ret < 0) {
		pr_err("%s : Failed to register switch device\n", __func__);
		goto err_switch_dev_register;
	}
...
err_request_detect_irq:
	...
	switch_dev_unregister(&switch_sendend_key);
