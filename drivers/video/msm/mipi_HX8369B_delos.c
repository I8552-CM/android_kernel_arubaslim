...
#define DIM_BRIGHTNESS_VALUE	31
...
static struct dsi_cmd_desc hx8369b_video_display_init_boe_cmds[] = {
	...
	{DTYPE_DCS_LWRITE, 1, 0, 0, 120, sizeof(hx8369b_boe14), hx8369b_boe14},
...
void ktd253_set_brightness(int level)
{
	...
	spin_lock_irqsave(&mipi_hx8369b_pdata->bl_spinlock, flags);		
	if (level > 0) {
		if (level < MIN_BRIGHTNESS_VALUE) {
			tune_level = DIM_BRIGHTNESS_VALUE; /* DIMMING */
		} else {
			int i;
	...
	mdelay(1);
	spin_unlock_irqrestore(&mipi_hx8369b_pdata->bl_spinlock, flags);
}
