...
static void max17048_get_soc(struct i2c_client *client)
{
	...
	if (psoc > empty_soc) {
		temp_soc = ((psoc - empty_soc) * full_soc)/(full_soc - empty_soc);
	...
	int temp, int chg_state)
{
	...
	if (chip->batt_type) { /* normal battery(4.2V) is 0 */
		...
		if (chg_state == POWER_SUPPLY_STATUS_CHARGING)
		{
			...
			chip->pdata->rcomp_value = 0x651c;
#elif defined(CONFIG_MACH_KYLEPLUS_CTC)    //rcomp tune in charging
			chip->pdata->rcomp_value = 0x771c;
#else
...
#if defined(CONFIG_MACH_KYLEPLUS_OPEN)
			chip->pdata->rcomp_value = 0x651c;
#elif defined(CONFIG_MACH_KYLEPLUS_CTC)    //rcomp tune in discharge
			chip->pdata->rcomp_value = 0x601c;
#else
			chip->pdata->rcomp_value = 0x501c;
#endif
		}
			...
			const struct i2c_device_id *id)
{
	...
	pr_info("%s: MAX17048 driver Loading!\n", __func__);
	...
	*/
	...
	ret = fuelgauge_create_attrs(chip->battery.dev);
	if (ret) {
		pr_err("%s : fuelgauge_create_attrs\n", __func__);
