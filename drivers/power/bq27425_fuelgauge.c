...
#if defined(CONFIG_MACH_ARUBA_OPEN) || defined(CONFIG_MACH_DELOS_OPEN) || defined(CONFIG_MACH_ARUBASLIM_OPEN)
#define FUEL_I2C_SCL 78
#define FUEL_I2C_SDA 79
#elif defined(CONFIG_MACH_ARUBA_CTC) || defined(CONFIG_MACH_DELOS_CTC) || defined(CONFIG_MACH_ARUBA_DUOS_CTC) || defined(CONFIG_MACH_KYLEPLUS_CTC) || defined(CONFIG_MACH_HENNESSY_DUOS_CTC)
...
unsigned int bq27425_get_average_vcell(unsigned int vcell)
{
...
unsigned int bq27425_get_vcell(void)
{
	...
	return bq27425_get_average_vcell(vcell);
}
...
/* For Detached full soc, calculate SOC value again after pull out the charger */
static int detached_rm = 1;
static int detached_fcc = 1;
void bq27425_set_charger_detached(void)
{	
	struct i2c_client *client = fg_i2c_client;

	detached_rm = bq27425_get_remcap();
	detached_fcc = bq27425_get_fullcap();	

	pr_info("[BATT] detached (rm:%d, fcc:%d)\n", detached_rm, detached_fcc);
}

unsigned int bq27425_get_recalc_soc(void)
{
	struct i2c_client *client = fg_i2c_client;
	int curr_rm, curr_fcc, soc;

	curr_rm = bq27425_get_remcap();
	curr_fcc = bq27425_get_fullcap();
	
	soc =  (curr_rm * detached_fcc * 101) / (curr_fcc * detached_rm);	
	...
	pr_info("[BATT] recalc_soc = %d, read_soc = %d (c_rm:%d, c_fcc:%d, d_rm:%d, d_fcc:%d\n", 
		soc, bq27425_get_soc(), curr_rm, curr_fcc, detached_rm, detached_fcc);
...
/* End For Detached full soc, calculate SOC value again after pull out the charger */
				...
				const struct i2c_device_id *id)
{
	...
	rc = gpio_tlmm_config(GPIO_CFG(FUEL_I2C_SDA , 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s : gpio_tlmm_config failed!(FUEL_I2C_SDA, %d)\n", __func__, rc);
		goto exit_gpio_tlmm_config_err;
	}
	rc = gpio_tlmm_config(GPIO_CFG(FUEL_I2C_SCL , 0, GPIO_CFG_OUTPUT,
		GPIO_CFG_PULL_UP, GPIO_CFG_2MA), GPIO_CFG_ENABLE);
	if (rc) {
		pr_err("%s : gpio_tlmm_config failed!(FUEL_I2C_SCL, %d)\n", __func__, rc);
		goto exit_gpio_tlmm_config_err;
	}
