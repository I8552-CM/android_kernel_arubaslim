...
//Check battery presence
  ...
  else
  {
    ...
    if (BattData.AvgVoltage<APP_MIN_VOLTAGE)
	  BattData.SOC = 0;
    else if (BattData.AvgVoltage<(APP_MIN_VOLTAGE+200) && BattData.AvgVoltage>(APP_MIN_VOLTAGE-500))
...
#if defined(CONFIG_BATTERY_STC3115_DELOS)
    GasGaugeData.SOC =  (GasGaugeData.SOC * 10250) / (99 * 100);   /* customer compensation */
#endif
    if (res>0) 
    {
        /* results available */
