...
#define INT_MASK_CMD_CHG_FUEL_ALERT 	0x6000
...
int multipdp_write(const unsigned char *, int);
int multipdp_dump(void);
void yhexdump(const char *buf, int len);
void multipdp_rx_noti_regi(int (*rx_func)(char *, int));
extern void power_down_timeout(unsigned long arg);
extern void power_down_registertimer(struct timer_list *ptimer,\
...
#endif
extern int subsystem_restart(const char *subsys_name);
