...
#if defined(CONFIG_MACH_ROY) || defined(CONFIG_TOUCHSCREEN_MELFAS_KYLE) || defined(CONFIG_TOUCHSCREEN_CYPRESS_TMA46X)
...
static ssize_t cpufreq_thermal_limit_show(struct kobject *kobj,
		struct kobj_attribute *attr, char *buf)
{
	int freq = (int)thermald_max_freq; 
	return sprintf(buf, "%d\n", freq);
}

static ssize_t cpufreq_thermal_limit_store(struct kobject *kobj,
					struct kobj_attribute *attr,
					const char *buf, size_t n)
{
	unsigned int freq;
	...
	sscanf(buf, "%8u", &freq);
	set_freq_limit(DVFS_THERMALD_ID, freq);
...
power_attr(cpufreq_thermal_limit);
...
#endif
#ifdef CONFIG_SEC_DVFS
	&cpufreq_thermal_limit_attr.attr,
